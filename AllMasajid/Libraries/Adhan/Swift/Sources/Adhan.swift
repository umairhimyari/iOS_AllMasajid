...
CalculationMethod {
    ...
    public var params: CalculationParameters {
        ...
        case .dubai:
            return CalculationParameters(fajrAngle: 18.2, ishaAngle: 18.2, method: self)
        case .other:
...
public struct PrayerTimes {
    ...
    public func time(for prayer: Prayer) -> Date? {
        ...
        case .isha:
            return isha
        case .ishraaq:
            return sunrise.addingTimeInterval(1200)
        case .chaasht:
             return sunrise.addingTimeInterval(6600)
        case .awabeen:
              return maghrib.addingTimeInterval(1200)
              //return maghrib.addingTimeInterval(120)
        case .tahajjud:
            let fajrComps = Calendar.current.dateComponents([.hour, .minute], from: fajr)
              let maghribComps = Calendar.current.dateComponents([.hour, .minute], from: maghrib)

            var maghribHour = (maghribComps.hour ?? 0)

            if maghribHour > 12 {
                maghribHour = maghribHour - 12
            }

            var hours : Double = Double((fajrComps.hour ?? 0) + maghribHour)
            var minutes = (fajrComps.minute ?? 0) + (maghribComps.minute ?? 0)
            if minutes >= 60 {
                hours+=1
                minutes = minutes - 60
            }
            hours = hours/3
            let interval = (-(TimeInterval(hours * 60 * 60) + TimeInterval(minutes * 60)))
            
            //let interval = (-(TimeInterval(3 * 60 * 60) + TimeInterval(38 * 60)))
            
            return fajr.addingTimeInterval(interval)
        }
    }
}
